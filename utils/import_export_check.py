...
from services.srv_to_import import pay_mode_all_data_other, pay_channel_all_data_other, cust_district_all_data_other
...
class dataImportCustDistrict(object):
    ...
    def check_file_data(self, file_row_data: list, row_num: int) -> List:
        """
        校验excel的每一行数据是否合格
        params file_row_data:每行数据，列表形式
               row_num:行数，从1开始计算的，此处为2表示表头是第一行
        ...
        # table_key_data = self.table_field_info[table_name_key]
        # 校验数据列数是否足够
        if len(file_row_data) < self.row:
            return [False, "文件中缺少【%s】列!" % (self.row - len(file_row_data))]
        # 判断每条数据的数据格式是否正确
        field_type_list = [int, int, basestring]
        for index, data in enumerate(file_row_data[0:self.row]):
            # 如果是整型数据会在读取的时候变成float类型，所以需要转
            is_type_correct = isinstance(data,
                                         ((int, float) if field_type_list[index] == int else field_type_list[index]))
            if not is_type_correct:
    ...
    def join_insert_sql(self, current_row_data):
        ...
        if self.flag_has_first:
            # 第一条数据创建插入语句
            self.sql = """
                   INSERT INTO cdp.cdim_cust_district (cust_district_code,ad_code,cust_district_name)
            VALUES {}
              """.format(tuple(current_row_data[0:self.row]))
            self.flag_has_first = False
        else:
            self.sql += ','
            self.sql += '{}'.format(tuple(current_row_data[0:self.row]))  # 对后续的数据拼接到插入语句

    def cust_district_check(self, table, cust_district_data, import_mode, cust_district_other) -> Dict:
        """
        支付方式验证
        params table:加载的excel数据
                pay_channel_data:支付方式的数据
        ...
        """
        for row_num in range(1, table.nrows):
            current_row_data = table.row_values(row_num)[0:self.row]  # 列表形式的每一行数据
            check_result_list = self.check_file_data(current_row_data, row_num + 1)  # 校验数据,返回的是True或False,校验结果信息字符串
            if not check_result_list[0]:  # 校验未通过
                return {"code": 0, "msg": check_result_list[1]}
            if import_mode in [1, '1']:
                # 已经有该编码数据并且支付方式修改了
                if (str(int(current_row_data[0])), str(int(current_row_data[1]))) in cust_district_data:
                    if (current_row_data[2],) not in cust_district_other:
                        update_sql_str = """
                            ALTER TABLE cdp.cdim_cust_district UPDATE cust_district_name = '{}' WHERE cust_district_code='{}' and ad_code='{}'
                        """.format(current_row_data[2], str(int(current_row_data[0])), str(int(current_row_data[1])))
                        self.update_sql_list.append(update_sql_str)
                # 没有该编码
                if (str(int(current_row_data[0])), str(int(current_row_data[1]))) not in cust_district_data:
                    self.join_insert_sql(current_row_data)
            elif import_mode in [2, '2']:
                # 忽略跳过
                if (str(int(current_row_data[0])), str(int(current_row_data[1]))) not in cust_district_data:
    ...
    def main(self, table, file, cust_district_data, import_mode) -> Dict:
        """
        数据入库
        """
        pay_channel_other = cust_district_all_data_other()
        sql_str = self.cust_district_check(table, cust_district_data, import_mode, pay_channel_other)
        if sql_str['code'] == 0:
            # 验证失败
            return {"code": 0, "msg": sql_str['msg']}
        try:
