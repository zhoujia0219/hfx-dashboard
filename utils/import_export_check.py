...
from services.srv_to_import import pay_mode_all_data_other
...
class dataImportPayMode(object):
    ...
    def check_pay_mode(self, table, pay_mode_data, import_mode,pay_mode_other) -> Dict:
        ...
        params table:加载的excel数据
                ...
                import_mode：导入方式，1表示有的替换导入，2表示有的数据则跳过，其他默认是增加
        return:
            {"code":0,msg:"","sql"}
            当通过校验则返回code为1,sql为拼接的sql字符串
            校验失败,则返回code为0,msg为未通过原因
        ...
        for row_num in range(1, table.nrows):
            ...
            #     # 第一条数据创建插入语句
            #     self.sql = """
            #            INSERT INTO cdp.cdim_pay_mode (pay_mode_name,pay_mode_code)
            # VALUES {}
            #       """.format(tuple(current_row_data[0:1]))
            #     self.flag_has_first = False
            # else:
            #     self.sql += ','
            #     self.sql += '{}'.format(tuple(current_row_data[0:7]))  # 对后续的数据拼接到插入语句
